...
public class Front {
    ...
    /**
     * Function that calculates the 2D area for the last two dimensions
     *
     * @param solutionList is a list of all the solutions on the pareto front
     * @param dimensionalIndicatorwill be 0 if there are only 2 dimensions to
     * each solution. It will be 1 if each solution has greater than 2
     * dimensions This is necessary for the remove dominated function so that it
     * knows what index to start at
     *
     * @return the area that the last 2 dimensions dominate
     */
     ...
     *
     * @param solutionList is a list of all the solutions on the pareto front
     * @param dimensionalIndicator will be 0 if there are only 2 dimensions to
     * each solution. It will be 1 if each solution has greater than 2
     * dimensions This is necessary for the remove dominated function so that it
     * knows what index to start at
     *
     * @return a list that has no solutions that are completely dominated by
     * another starting from the second index in the list assuming it has more
     * than 2 dimensions. In the 2 dimensional case it returns a list with no
     * completely dominated solutions starting from the first index
     */
    ...
    /**
     * Function to calculate the total area dominated by all the solutions in
     * the list
     *
     * @param solutionList is a list of all the solutions on the pareto front
     * 
            ...
            if (i != 0) {
                for (int j = 0; j < listOfSolutions.size(); j++) {
                    if (j == 0) {
                        ...
                        dominate += areaList.get(j) * listOfSolutions.get(j).get(i - 1);
                    } else {
                         ...
                         */
                        dominate += areaList.get(j) * (listOfSolutions.get(j).get(i - 1) - listOfSolutions.get(j - 1).get(i - 1));
                    }
                }
